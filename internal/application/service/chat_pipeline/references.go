...
func prepareMessagesWithReferences(
	...
	if chatManage != nil {
		supportsVision = chatManage.ChatModelSupportsVision
	}
	return prepareMessagesWithReferencesForVision(ctx, chatManage, supportsVision)
}

// prepareMessagesWithReferencesForVision prepares the same request-local
// source protocol as prepareMessagesWithReferences while adapting the user
// message to the model that will actually receive it. This matters when a
// fallback model has different vision capabilities from the primary model.
func prepareMessagesWithReferencesForVision(
	ctx context.Context,
	chatManage *types.ChatManage,
	supportsVision bool,
) ([]chat.Message, *llmreference.Registry) {
	...
	messages := prepareMessagesWithHistoryForVision(chatManage, supportsVision)
	if len(messages) > 0 {
...
// prepareEncodedMessagesWithReferences creates all request-local registries
// used for one model attempt and returns messages encoded for that attempt.
// Callers switching to a fallback model must also switch to the returned
// registries so the response is decoded against the exact prompt it saw.
func prepareEncodedMessagesWithReferences(
	ctx context.Context,
	chatManage *types.ChatManage,
	supportsVision bool,
) ([]chat.Message, *llmreference.Registry, *llmresource.Registry) {
	messages, sourceRefs := prepareMessagesWithReferencesForVision(ctx, chatManage, supportsVision)
	resourceRefs := llmresource.NewRegistry()
	messages = sourceRefs.EncodeMessages(messages)
	messages = resourceRefs.EncodeMessages(messages)
	return messages, sourceRefs, resourceRefs
}
