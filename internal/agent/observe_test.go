...
func TestAppendToolResults_AddsDynamicImageRequirementToCustomSystemPrompt(t *testing.T) {
	...
	assert.Contains(t, out[0].Content, "Do not summarize, shorten, paraphrase, merge, or omit")
	assert.Contains(t, out[0].Content, "same complete Chinese caption text")
	assert.Contains(t, out[0].Content, `same patient as Fig. X`)
	assert.Contains(t, out[0].Content, "Do not add interpretation, inference")
