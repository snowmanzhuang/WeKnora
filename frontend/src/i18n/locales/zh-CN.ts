export default {
  ...
  agent: {
    ...
    editor: {
      ...
      fallbackModel: "备用模型",
      fallbackModelDesc: "主模型不可用且尚未输出回答时自动切换。模型连接信息请在模型设置中维护。",
      fallbackModelPlaceholder: "可选，请选择备用模型",
      fallbackModelSameAsPrimary: "备用模型不能与主模型相同",
      systemPrompt: "系统提示词",
  ...
  agentStream: {
    ...
    tools: {
      ...
      researchDifferentials: "并行研究鉴别诊断",
      differentialSubagent: "鉴别诊断检索子智能体",
    ...
    toolStatus: {
      ...
      differentialResearchRunning: "正在并行研究 {count} 个鉴别方向...",
      differentialResearchDone: "已完成 {completed}/{count} 个鉴别方向，找到 {images} 张参考图",
      differentialSubagentRunning: "正在检索：{diagnosis}",
      differentialSubagentDone: "{diagnosis}：证据与图片检索完成",
      differentialSubagentNoImage: "{diagnosis}：证据检索完成，暂无可靠配图",
      differentialSubagentNoMatch: "{diagnosis}：未找到足够相关证据",
      differentialSubagentFailed: "{diagnosis}：子智能体执行失败",
  ...
  agentEditor: {
    ...
    imageUpload: {
      ...
      auxiliaryPreanalysisLabel: "启用辅助 VLM 预解析",
      auxiliaryPreanalysisDesc: "仅用于智能推理：辅助 VLM 先生成眼科图像解析报告，主模型随后同时读取原图和该报告并执行 ReAct 与知识库检索。会增加一次模型调用和响应时间。",
      auxiliaryPreanalysisVisionRequired: "主对话模型必须开启“支持视觉”才能使用辅助 VLM 预解析",
      differentialSubagentsLabel: "启用鉴别诊断并行子智能体",
      differentialSubagentsDesc: "仅用于智能推理图片问诊：主模型确定 2–5 个鉴别方向后，为每个方向并发检索知识库证据和同类型参考影像，再统一汇总。",
      differentialSubagentModel: "子智能体模型",
      differentialSubagentModelDesc: "用于归纳每个鉴别方向的独立检索结果。建议选择响应较快、成本较低的模型；留空则复用主对话模型。",
      differentialSubagentModelPlaceholder: "留空则使用主对话模型",
      differentialSubagentConcurrency: "最大并发子智能体数",
      differentialSubagentConcurrencyDesc: "可设置 1–5；服务端会强制限制为最多 5 路并发。",
