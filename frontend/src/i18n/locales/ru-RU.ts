export default {
  ...
  agent: {
    ...
    editor: {
      ...
      fallbackModelDesc: 'Автоматически включается, если основная модель недоступна и ответ ещё не начат. Данные API настраиваются в разделе моделей.',
      fallbackModelPlaceholder: 'Необязательно: выберите резервную модель',
      fallbackModelSameAsPrimary: 'Резервная модель должна отличаться от основной',
...
/example\.(net|org)/`,
  ...
  agentStream: {
    ...
    tools: {
      ...
      researchDifferentials: 'Параллельный поиск дифференциальных диагнозов',
      differentialSubagent: 'Субагент дифференциального поиска',
    ...
    toolStatus: {
      ...
      differentialResearchRunning: 'Параллельно исследуется направлений: {count}...',
      differentialResearchDone: 'Завершено {completed}/{count}, выбрано изображений: {images}',
      differentialSubagentRunning: 'Поиск: {diagnosis}',
      differentialSubagentDone: '{diagnosis}: доказательства и изображения найдены',
      differentialSubagentNoImage: '{diagnosis}: доказательства найдены, надёжного изображения нет',
      differentialSubagentNoMatch: '{diagnosis}: недостаточно релевантных данных',
      differentialSubagentFailed: '{diagnosis}: ошибка субагента',
  ...
  agentEditor: {
    ...
    imageUpload: {
      ...
      auxiliaryPreanalysisLabel: 'Включить предварительный анализ вспомогательной VLM',
      auxiliaryPreanalysisDesc: 'Только для умного рассуждения: вспомогательная VLM сначала создаёт структурированный отчёт, затем основная модель получает исходное изображение и отчёт для ReAct и поиска по базе знаний. Это добавляет один вызов модели и задержку.',
      auxiliaryPreanalysisVisionRequired: 'Для этой функции основная языковая модель должна поддерживать ввод изображений',
      differentialSubagentsLabel: 'Параллельные субагенты дифференциальной диагностики',
      differentialSubagentsDesc: 'Для изображений в режиме Smart Reasoning: после выбора 2–5 направлений отдельные исполнители параллельно ищут доказательства и эталонные изображения.',
      differentialSubagentModel: 'Модель субагента',
      differentialSubagentModelDesc: 'Суммирует независимый поиск по каждому направлению. Если поле пустое, используется основная модель.',
      differentialSubagentModelPlaceholder: 'Использовать основную модель',
      differentialSubagentConcurrency: 'Максимум параллельных субагентов',
      differentialSubagentConcurrencyDesc: 'Допустимо 1–5; сервер всегда ограничивает параллелизм пятью.',
