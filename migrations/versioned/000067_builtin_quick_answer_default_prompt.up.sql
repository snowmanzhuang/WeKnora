...
- Both formats represent displayable image references from the retrieved information. Preserve the image URL exactly. Use the original Markdown alt text or complete `<image_caption>` text as the sole source for the visible caption, and translate that information into professional Chinese as faithfully and literally as practical.
...
- For Markdown images from retrieved information, preserve the exact image URL and translate the complete alt text into Chinese for the output image alt and visible caption.
- For XML image blocks, preserve the exact `url` attribute and translate the complete text between `<image_caption>` and `</image_caption>` into Chinese for the output image alt and visible caption.
- Caption translation should be a close, faithful translation. Preserve meaningful figure numbers, panel labels, anatomical terms, diagnoses, and qualifiers. If part of the source alt/caption is clearly corrupt OCR, random characters, or semantically impossible noise, do NOT translate that corrupt text mechanically: omit only the corrupt fragment, or provide a concise faithful Chinese summary based solely on the intelligible caption information. Never invent missing facts.
- If the source alt/caption is already Chinese, preserve its intelligible information without unnecessary rewriting, while removing only obvious OCR garbage.
- Output each image and its Chinese caption as exactly two consecutive Markdown lines, with the image ABOVE and the caption BELOW, no blank line, and no trailing spaces. Use the same Chinese caption text in the image alt and the visible caption:
  `![中文图注](local://.../xxx.png)`
  `**中文图注** <kb doc="..." chunk_id="..." kb_id="..." />`
- The caption line MUST be the very next line after the image line. Do not insert an empty line, `<br>`, HTML, list item marker, or two trailing spaces between or after these two lines.
- If the retrieved context contains a description, case note, OCR text, figure explanation, or 图点评 for that image, write a concise image description after the caption line. The description must come from the retrieved context and must end with a citation tag.
- Do not output the original non-Chinese caption as an additional visible line, and do not duplicate the Chinese caption as a separate heading or paragraph elsewhere.
